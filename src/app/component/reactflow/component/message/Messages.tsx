  ...
  return (
    <div className={'f-messages'}>
      {messages?.map((msg, index) => (
        <div key={index}>
          <div
            className={`rounded-md shadow-md whitespace-pre-line min-h-10 pl-4 pt-2 pr-4 pb-2 mb-2 bg-white ${index === editingIndex ? 'bg-yellow-400' : ''}`}>
            ...
            }
            {
              msg?.type === 'IMAGE' && (
                <div className={'flex flex-col'}>
                  <img src={msg?.link} alt=''
                       className={'w-full h-32 rounded-md mb-2 bg-no-repeat bg-center object-cover'}/>
            ...
            }
            {
              msg?.type === 'VIDEO' && (
                <div className={'flex flex-col'}>
                  <video className={'w-full h-32 rounded-md mb-2 bg-no-repeat bg-center object-cover'} src={msg?.link}
                         controls></video>
                  <div className={'mt-auto'}>{msg?.body}</div>
                </div>
              )
            }
            {
              msg?.type === 'DOCUMENT' && (
                <div className={'flex flex-col'}>
                  <embed className={'w-full h-32 rounded-md mb-2 bg-no-repeat bg-center object-cover'} src={msg?.link}/>
                  <div className={'mt-auto'}>{msg?.body}</div>
                </div>
              )
            }

...
function Buttons(props: Props) {
  ...
  return (
    <div className={'f-buttons flex flex-col pb-2 border-b xyflow-node-background-color'}>
      {
        buttons?.map((button, index) => (
          <div className={'flex flex-row relative'} key={button.id}>
            <div className={'btn-whatsapp flex flex-row mt-2 ml-4 mr-4 h-8 rounded-md bg-white w-full'}>
              {buttons.length > 1 && (
              ...
              <input type='text'
                     className={'outline-none px-4 text-center rounded-md w-full'}
